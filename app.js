...
function renderReview() { const review = document.querySelector("#review"); if (!review) return; const reviews = state.reviews.map(review => `<article class="review-item"><div><strong>${esc(review.source)}</strong><span>Question ${esc(review.question)}</span></div><div class="review-meta"><span>Chosen: ${esc(review.chosen)}</span><span>Correct: ${esc(review.correct)}</span><span>${esc(review.reason)}</span></div><p><strong>Why I chose it:</strong> ${esc(review.whyChosen)}</p><p><strong>Explanation:</strong> ${esc(review.explanation)}</p><button type="button" class="btn link" data-delete-review="${review.id}">Delete review</button></article>`).join(""); review.innerHTML = `<div class="page-head"><div><div class="page-kicker">Review</div><h1>Question review</h1></div></div><section class="card review-card"><p class="muted">Capture the reasoning behind missed or uncertain questions so the same pattern is easier to fix.</p><div class="review-form"><label>Exam or QPack<input id="review-source" placeholder="AAMC FL1, UWorld C/P, Biology QPack"></label><label>Question #<input id="review-question" placeholder="42"></label><label>Chosen answer<input id="review-chosen" placeholder="B"></label><label>Correct answer<input id="review-correct" placeholder="D"></label><label class="review-wide">Why did you choose it?<textarea id="review-why" rows="2"></textarea></label><label>Why do you think you got it wrong?<select id="review-reason"><option>Content gap</option><option>Misread the question</option><option>Careless mistake</option><option>Fatigue</option><option>Timing pressure</option><option>Knowledge application</option><option>Other</option></select></label><label class="review-wide">Explanation to review later<textarea id="review-explanation" rows="3"></textarea></label><button type="button" class="btn primary" data-add-review>Add review</button></div></section><div class="review-list">${reviews || '<p class="muted">No question reviews saved yet.</p>'}</div>`; }
...
function prunePostExamTasks() { const exam = parseDate(state.examDate); if (!exam) return; const examKey = dateKey(exam); let changed = false; Object.keys(state.tasks).forEach(key => { if (key >= examKey) { delete state.tasks[key]; changed = true; } }); if (changed) saveState(); }
function renderToday() {
  ...
  const checklist = tasks.length ? tasks.map((task, index) => taskMarkup({ ...task, index }, true)).join("") : '<p class="muted">No assignments for today.</p>';
  ...
  document.querySelector("#today").innerHTML = `
    ...
    <section class="daily-summary"><div><strong>${questions}</strong><span>questions today</span></div><div><strong>${passages}</strong><span>passages today</span></div><div><strong>${resources}</strong><span>resources</span></div></section>
    <div class="dashboard">
...
function taskMarkup(task, checkbox) { const resource = task.section === "Jack Westin" ? '<button type="button" class="task-resource" data-link="https://jackwestin.com/#daily-passages">Open daily passage</button>' : ""; return `<label class="task ${sectionClass(task.section)}"><input type="checkbox" ${task.done ? "checked" : ""} data-task-date="${task.date}" data-task-index="${task.index}"><span><strong>${esc(task.label)}</strong>${resource}</span></label>`; }
